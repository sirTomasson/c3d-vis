...
# Ensure TensorFlow 2.x is running in TensorFlow 1.x compatibility mode
tf.compat.v1.disable_eager_execution()  # Disable eager execution for TF 1.x style

# Keep tf_compat reference for clarity in the original code
...
# Import these from your "common" module or define them here
# from common import showarray, visstd, display_videos
# Define placeholders if missing
def showarray(img):
    # Placeholder for the original showarray function
    pass


def visstd(img):
    # Placeholder for the original visstd function
    pass


def display_videos(videos):
    # Placeholder for the original display_videos function
    pass
...
def render_vis(model, objective_f, param_f=None, optimizer=None,
    ...
    with tf_compat.Graph().as_default() as graph, tf_compat.Session() as sess:

        if use_fixed_seed:  # does not mean results are reproducible, see Args doc
            tf_compat.set_random_seed(0)
        ...
        tf_compat.global_variables_initializer().run()
        ...
        try:
            ...
            for i in bar:
                ...
                if i in thresholds:
                    print('HERE')
                    vis = t_image.eval(session=sess)  # Explicitly provide session
                    images.append(vis)
                    if verbose:
...
def render_vis_explore(model, objective_f, param_f=None, optimizer=None,
    ...
    with tf_compat.Graph().as_default() as graph, \
            tf_compat.Session() as sess, tqdm() as pbar:
...
# Define handle_batch if it's not already defined
...
# Define _dot_cossim if it's not already defined
def _dot_cossim(acts, vec, cossim_pow=0):
    dot = tf_compat.reduce_sum(acts * vec)
    if cossim_pow == 0:
        return dot
    mag = tf_compat.sqrt(tf_compat.reduce_sum(vec ** 2))
    return dot / mag ** cossim_pow
    ...
    def inner(T):
        ...
        for d in [1]:
            for i in range(frames_n - d):
                ...
                accum += tf_compat.reduce_mean((arr1 - arr2) ** 2) / decay_ratio ** float(d)
        return -accum
...
def to_herm_nd_tf(s):
    ...
    """
    cat = lambda a, b, axis: tf_compat.concat([a, b], axis)
    ...
    def sub_and_filp(s, axis):
        ...
        return tf_compat.reverse(x, axis=[axis])

    skip_last = int(s.shape[-1]) % 2 == 1
    s1 = tf_compat.reverse(s[..., slice(1, int(s.shape[-1]) - skip_last)], axis=[-1])
    ...
    return cat(s, tf_compat.conj(r), -1)


def fft_video(shape, sd=None, decay_power=1, rand=False):
    sd = sd or 0.01  # Provide default value
    ...
    for _ in range(batch):
        if rand:
            spectrum_real_imag_t = sd * tf_compat.random_normal(init_val_size, dtype="float32")
        else:
            init_val = np.random.normal(size=init_val_size, scale=sd).astype(np.float32)
            spectrum_real_imag_t = tf_compat.Variable(init_val)

        spectrum_t = tf_compat.complex(spectrum_real_imag_t[0], spectrum_real_imag_t[1])
        ...
        # Update for TF 2.x - use the new spectral namespace
        # TF 2.x doesn't have tf.spectral.ifft3d, use tf.signal.ifft3d
        image_t = tf.signal.ifft3d(scaled_spectrum_t)
        image_t = tf_compat.transpose(image_t, (1, 2, 3, 0))
        image_t = tf_compat.real(image_t)

        image_t = image_t[:t, :h, :w, :ch]
        images.append(image_t)

    batched_image_t = tf_compat.stack(images) / 4.0  # TODO: is that a magic constant?
...
def pixel_video(shape, sd=None, init_val=None):
    ...
    return tf_compat.Variable(init_val)
...
def uniform_video(t, w, h=None, batch=None, offset=100.):
    ...
    x = tf_compat.Variable(np.random.uniform(size=shape).astype(np.float32) + offset)
    return tf_compat.identity(x)
...
def video(t, w, h=None, batch=None, sd=None, decorrelate=True, fft=True, alpha=False):
    ...
    if alpha:
        a = tf_compat.nn.sigmoid(t[..., 3:])
        return tf_compat.concat([rgb, a], -1)
    return rgb
...
def pad_with_t(w, t, mode="REFLECT", constant_value=0.5):
    def inner(t_image):
        if constant_value == "uniform":
            constant_value_ = tf_compat.random_uniform([], 0, 1)
        else:
            constant_value_ = constant_value
        return tf_compat.pad(
            t_image,
            [(0, 0), (t, t), (w, w), (w, w), (0, 0)],
            mode=mode,
...
def wrap_transform(transform, levels):
    def fn(video, its, transform):
        video = tf_compat.convert_to_tensor(video, preferred_dtype=tf_compat.float32)
        return tf_compat.map_fn(transform, video,
...
# Import pad from lucid.optvis.transform
from lucid.optvis.transform import pad

# Import ops from TensorFlow
from tensorflow.python.framework import ops
from tensorflow.python.ops import array_ops, math_ops, control_flow_ops, random_ops


def jitter_image(t_image, seed, d):
    t_image = tf_compat.convert_to_tensor(t_image, preferred_dtype=tf_compat.float32)
    t_shp = tf_compat.shape(t_image)
    crop_shape = tf_compat.concat([t_shp[:-3], t_shp[-3:-1] - d, t_shp[-1:]], 0)
    if seed:
        tf_compat.set_random_seed(seed)
    # Need to use tf.random.crop instead of tf.random_crop in newer TF
    crop = tf_compat.random_crop(t_image, crop_shape, seed=seed)
...
def _angle2rads(angle, units):
    angle = tf_compat.cast(angle, "float32")
...
def _rand_select(xs, seed=None):
    ...
    if seed:
        tf_compat.set_random_seed(seed)
    rand_n = tf_compat.random_uniform((), 0, len(xs_list), "int32", seed=seed)
    return tf_compat.constant(xs_list)[rand_n]


def random_scale_image(t, seed, scales):
    t = tf_compat.convert_to_tensor(t, preferred_dtype=tf_compat.float32)
    scale = _rand_select(scales, seed=seed)
    shp = tf_compat.shape(t)
    scale_shape = tf_compat.cast(scale * tf_compat.cast(shp[-3:-1], "float32"), "int32")
    # Update image resize function for TF 2.x
...
def random_rotate_image(t, seed, angles, units="degrees"):
    ...
    # Update for TF 2.x - use tf.image.rotate instead of tf.contrib.image.rotate
...
def hang_video(subclip_len=10):
    def inner(video):
        ...
        b = tf_compat.random_uniform([1], minval=0, maxval=clip_len - subclip_len, dtype=tf_compat.int32)[0]
        e = b + subclip_len

        return tf_compat.concat([
            video[:, :b, :, :, :],
            tf_compat.tile(video[:, b:b + 1, :, :, :], [1, e - b, 1, 1, 1]),
            video[:, e:, :, :, :]], axis=1)
...
def lap_normalize(img, scale_n=4, k0=[1, 4, 6, 4, 1]):
    ...
    def lap_split(img):
        ...
        with tf_compat.name_scope('split'):
            lo = tf_compat.nn.conv3d(img, k5x5x5, [1, 2, 2, 2, 1], 'SAME')
            lo2 = tf_compat.nn.conv3d_transpose(lo, k5x5x5 * 4, tf_compat.shape(img), [1, 2, 2, 2, 1])
            hi = img - lo2
        return lo, hi
    ...
    def lap_merge(levels):
        ...
        img = levels[0]
        for hi in levels[1:]:
            with tf_compat.name_scope('merge'):
                img = tf_compat.nn.conv3d_transpose(img, k5x5x5 * 4, tf_compat.shape(hi), [1, 2, 2, 2, 1]) + hi
